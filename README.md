...
한편으로는 구조체의 이름이 중복되기 때문에 일어난 일이기 때문에 구조체의 이름을 **FGameInfoStruct**로 바꿔주어서 해결하였습니다.
...
추가적으로 int8 자료형 변수에 BlueprintReadWrite속성을 넣을 경우 에러가 발생합니다. 해당 구조체는 블루프린트에 노출이 필요 없으므로 USTRUCT()만 남겨놓아 GC에 추가 할 수만 있게 만들겠습니다.
