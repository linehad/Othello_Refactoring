...
```
충돌이 일어난 구조체입니다. 오류를 해결하는 방법으로 구조체의 이름이 중복되기 때문에 일어난 일이기 때문에 구조체의 이름을 **FGameInfoStruct**로 바꿔주었으나,
근본적으로는 **Python Editor Script Plugin**때문이므로 해당 플러그인 또한 비활성화 시켜서 추가적인 문제를 해결하였습니다.
