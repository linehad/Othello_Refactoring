...
{	
	...
	{
		AOthelloGameModeBase* GameMod = Cast<AOthelloGameModeBase>(UGameplayStatics::GetGameMode(GetWorld()));
		ASinglePlayGameMode* SingleGameMod = Cast<ASinglePlayGameMode>(UGameplayStatics::GetGameMode(GetWorld()));
		if (GameMod)
		{
			GameMod->SetGameData(size, time);
		}
		else if (SingleGameMod)
		{
			SingleGameMod->SetGameData(size, time);
		}
